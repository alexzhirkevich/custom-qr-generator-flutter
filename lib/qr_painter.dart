...
class QrPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (options.colors.frame is! QrColorUnspecified) {
      ...
      fullDarkPath.addPath(framePath, Offset(pixelSize * (matrix.width-7), 0));
    }
    if (options.colors.ball is! QrColorUnspecified) {
      canvas.translate(pixelSize * 2, pixelSize * 2);
      canvas.drawPath(ballPath, ballPaint);
    } else {
      fullDarkPath.addPath(ballPath, Offset(pixelSize * (matrix.width-5), pixelSize * 2));
    }
    canvas.restore();
    ...
    if (options.colors.frame is! QrColorUnspecified) {
      ...
      fullDarkPath.addPath(framePath, Offset(0,pixelSize * (matrix.width-7)));
    }
    if (options.colors.ball is! QrColorUnspecified) {
      canvas.translate(pixelSize * 2, pixelSize * 2);
      canvas.drawPath(ballPath, ballPaint);
    } else {
      fullDarkPath.addPath(ballPath, Offset(pixelSize * 2, pixelSize*( matrix.height-5)));
    }
    canvas.restore();
